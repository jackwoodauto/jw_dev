		...
		if($_FILES['input_quote']['type']=="application/pdf"){
			...
			*/
			...
			for ($image_count=1; $image_count <= $number_of_jpgs; $image_count++) {
				...
				echo "analysing jpg image: ".$upload_folder.$jpg_filename;

				//put this in a function:

				$result_of_conversion = post_image_to_vision($ch, $upload_folder.$jpg_filename);

				echo "results:<br><pre>";
				print_r($result_of_conversion);
				echo "</pre>";
				...
				$quote_text .= $result_of_conversion;
			}
			curl_close($ch);
...
die();
			...
			/*
			 if jpg in folder $destinationJPGname -1 -2 -3
			post_images_to_vision(jpg1,jpg2 etc)
			else {
				echo "failed";

			$str1 = Strlen($return_pdf_info);
			$str2 = $str1 - 4;
			$str2 = substr($return_pdf_info,$str1,$str2 )
			$actualljpg = $str2.".jpg";
			*/
// $count_jpg = count(glob("pdfs/*uniquenumber/*.jpg"));  //unique returned id ($return_pdf_info)
// while $count_jpg =< 0
...
echo "</pre>";
	...
	if($_FILES['input_invoice']['name'][0]!=""){
		if($_FILES['input_invoice']['type']=="application/pdf"){
			upload_pdf_and_convert_to_jpg("invoice");
		}
		else {
			$arr['dest_file_folder'] = "/quotecheck/uploads/";
			...
			$result = upload_file($arr, $_FILES['input_invoice']);
